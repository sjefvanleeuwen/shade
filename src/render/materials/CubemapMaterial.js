export class CubemapMaterial {
    constructor(engine, options = {}) {
        ...
        this.id = options.id || `cubemap_${Math.floor(Math.random() * 10000)}`;
        this.type = 'cubemap';
        this.texture = null;
        this.bindGroup = null;
    ...
    async loadShader() {
        try {
            // Create an inline shader instead of loading from a file
            // This eliminates the need for a network request
            const shaderCode = `
...
fn fragmentMain(input: VertexOutput) -> @location(0) vec4<f32> {
    // Compute cubic texture mapping
    // Determine which face we're on and compute 2D texture coordinates
    var texCoord: vec2<f32>;
    let p = normalize(input.worldPos);
    let absP = abs(p);
    
    // Find which component has the largest magnitude to determine which face we're on
    if (absP.x >= absP.y && absP.x >= absP.z) {
        // X-axis face
        texCoord = vec2<f32>(p.z / p.x * 0.5 + 0.5, p.y / p.x * 0.5 + 0.5);
        if (p.x < 0.0) {
            texCoord.x = 1.0 - texCoord.x;  // Flip for negative X
        }
    } else if (absP.y >= absP.x && absP.y >= absP.z) {
        // Y-axis face
        texCoord = vec2<f32>(p.x / p.y * 0.5 + 0.5, p.z / p.y * 0.5 + 0.5);
        if (p.y < 0.0) {
            texCoord.y = 1.0 - texCoord.y;  // Flip for negative Y
        }
    } else {
        // Z-axis face
        texCoord = vec2<f32>(p.x / p.z * 0.5 + 0.5, p.y / p.z * 0.5 + 0.5);
        if (p.z < 0.0) {
            texCoord.x = 1.0 - texCoord.x;  // Flip for negative Z
        }
    }
    
    // Sample the texture with computed coordinates
    let texColor = textureSample(colorTexture, textureSampler, texCoord);
    ...
    return texColor;
            ...
            console.log("Cubemap shader initialized successfully");
            
        } catch (error) {
            console.error("Failed to initialize cubemap shader:", error);
            throw error;
        }
    }
    ...
    createModelBindGroup() {
        ...
        console.log("Created model bind group for cubemap material:", this.modelBindGroup);
    }
    ...
    setTexture(texture) {
        ...
        if (this.pipeline && this.texture) {
            ...
            console.log("Created texture bind group for cubemap material:", this.bindGroup);
        } else {
